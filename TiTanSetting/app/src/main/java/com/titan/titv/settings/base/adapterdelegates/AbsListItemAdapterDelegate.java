package com.titan.titv.settings.base.adapterdelegates;
...
public abstract class AbsListItemAdapterDelegate<I extends T, T, VH extends RecyclerView.ViewHolder>
  ...
  @Override
  protected final boolean isForViewType(List<T> items, int position) {
    return isForViewType(items.get(position), items, position);
  }

  @Override
  protected final void onBindViewHolder(List<T> items, int position,
      RecyclerView.ViewHolder holder, List<Object> payloads) {
    if (payloads == null || payloads.isEmpty()) {
      onBindViewHolder((I) items.get(position), (VH) holder, position);
    } else {
      onBindViewHolder((I) items.get(position), (VH) holder, position, payloads);
    }
  }
   ...
   */
  protected abstract boolean isForViewType(T item, List<T> items, int position);
   ...
   * @return ViewHolder
   */
  @Override protected abstract VH onCreateViewHolder(ViewGroup parent);
   ...
   */
  protected abstract void onBindViewHolder(I item, VH viewHolder, int pos);
   ...
   */
  protected abstract void onBindViewHolder(I item, VH viewHolder, int pos, List<Object> payloads);
}
